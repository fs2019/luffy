    ...
    methods: {
      loginHander() { // 用户密码账号登录
        this.$axios.post(`${this.$settings.HOST}/user/login/`, {
          ...
          } else { // 不记住登录状态，sessionStorage 浏览器关闭就失效，localStorage 长期有效
      ...
      get_geetest_captcha() { // 获取验证码【滑动验证】
        this.$axios.get(`${this.$settings.HOST}/user/captcha/`, {
          params: {
            username: this.username,
          }
        }).then(response => { // 使用initGeetest接口，
          // 参数1：配置参数 ； 参数2：回调，回调的第一个参数验证码对象，之后可以使用它做appendTo之类的事件
      ...
      handlerPopup(captchaObj) {
        ...
        captchaObj.onSuccess(function () {
          ...
          self.$axios.post(`${self.$settings.HOST}/user/captcha/`, {
            ...
            if (response.data.status) { // 验证码通过以后，才发送账号和密码进行登录！
              self.loginHander();
            }
          }).catch(error => {
            console.log(error.response);
